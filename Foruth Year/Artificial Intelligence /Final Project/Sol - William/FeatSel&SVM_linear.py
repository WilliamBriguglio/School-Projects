...
##remove constant features and their corresponding ID
...
GeneIDs = GeneIDs[sfm.get_support(indices=True)] #select corresponding gene ID's

np.savetxt('SFMGeneIDs_T4.csv', GeneIDs, delimiter=",", fmt='%s');	#save gene ID's
np.savetxt('SFMFeats_T4.csv', X_transform, delimiter=",");	#save genes
...
f=open("Scores_linear_T4_throw.txt", "w+"); #open file for saving scores and confusion matrices
...
for n in Ns: 
	...
	#np.savetxt('Linear_T4.csv', GeneIDsRFE, delimiter=",", fmt='%s'); #save feature ID's
	...
	Y_pred = cross_val_predict(SVM, XRFE, Y2, cv = 3)#obtain prediction results with best params	
